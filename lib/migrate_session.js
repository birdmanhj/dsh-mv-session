...
function usage() {
  return [
    ...
    '  --verify             read-only consistency check: registry record <-> session headers <-> dirs <-> cache',
 ...
 * "restart to confirm" step: after the (single, required) restart and the
 * symlink cleanup, this asserts that the four data layers agree —
 * workspace.json record, session header cwd + frame invariant, sessions
 * directory location, and projection-cache identity. A stale cache cwd or a
 * leftover transition symlink is reported as a WARNING (both are harmless and
 * self-healing), everything else as a PROBLEM.
 */
async function verifyHome(opts, z) {
  ...
  if (dis) {
    ...
    for (const s of dis.sessions) {
      const entry = pcSessions && pcSessions[s.sid];
      if (entry && entry.identity && typeof entry.identity.cwd === 'string' && entry.identity.cwd !== dis.wsRec.path) {
        warnings.push('projcache cwd stale for ' + s.sid + ' (' + entry.identity.cwd + ') — self-heals via cold rebuild on read, harmless');
      }
    }
  ...
  return { ok: problems.length === 0, problems, warnings, checks };
}
...
async function buildPlan(dis, opts, z) {
  ...
  // 5. projcache update
  for (const s of dis.sessions) {
    plan.push({ step: 'update_projcache_cwd', sid: s.sid, newCwd: dis.toCwd });
  }
  ...
  plan.push({
    step: 'manual',
    detail: 'restart dsh web; after the GUI confirms the new workspace, delete the transition symlinks: ' + dis.fromCwd + ' and ' + fromProj + '; then close the loop with: node ' + selfPath + ' --verify --from ' + JSON.stringify(dis.toCwd),
  });
  return plan;
}
...
async function executePlan(dis, plan, opts, z, backupDir) {
  ...
  for (const p of plan) {
    switch (p.step) {
      ...
      case 'update_projcache_cwd': {
        if (!dis.projcache || !dis.projcacheFile) break;
        const entry = dis.projcache.tables && dis.projcache.tables.sessions && dis.projcache.tables.sessions[p.sid];
        if (entry && entry.identity) {
          entry.identity.cwd = p.newCwd;
          writeJson(dis.projcacheFile, dis.projcache);
        }
        break;
      }
      case 'cleanup_empty_sessions': {
        ...
        for (const e of empties) {
          ...
          if (dis.projcache && dis.projcache.tables && dis.projcache.tables.sessions) {
            delete dis.projcache.tables.sessions[e.sid];
          }
        }
        if (empties.length) {
          writeJson(dis.workspaceFile, state);
          if (dis.projcacheFile) writeJson(dis.projcacheFile, dis.projcache);
...
async function main() {
  ...
  if (opts.help || (!opts.from && !opts.session) || (!opts.verify && !opts.to)) {
...
if (require.main === module) {
  ...
  module.exports = {
    ...
    rewriteHeaderCwd, verifyMigratedLog, verifyHome, preflightLogs, projectKey, encodeSegment, parseArgs,
  };
}
