...
> *EN: Internal mechanics of DSH session/workspace migration — persistence layout, the zstd
> frame invariant, in-process memory behavior, risk list, and source locations. The full English
> user manual lives at [docs/user-manual.en.md](user-manual.en.md).*
>
> 来源：2026-08-24 一次真实工作区改名迁移中，通过阅读 DSH 源码（`@deepseek-ai/dsh-*` 包）
> 与实际操作验证得到。开发/维护 `mv-session` 插件前必读。
...
例：`/home/<you>/workspaces/my_project`
→ `--home--you--workspaces-my~005Fproject--`
（注意：实际观察到的目录名里 `_` 保持原样，说明该版本编码规则允许 `_`；
...
- **损坏形态**（本次事故）：旧的 `zstd -dc | 编辑 | zstd -q -c` 整文件往返会把数千行压成**一个**巨帧，
  帧 0 不再是一行 → boot 崩溃。数据本身完好，只是帧结构错位。
- **修复工具**：社区修复脚本 `fix_dsh_frames_node.mjs`（deepseek-harness 技能包内）
...
- **迁移脚本的正确做法**（`lib/migrate_session.js` 已实现）：
  ...
  5. 变更前预检（`preflightLogs`）：坏日志在**任何改动前**中止（连备份都不写）；
     塌缩但可修复的日志照常放行；
  6. 回归测试 `tests/migrate_e2e_scratch.js`：两种布局（正常多帧 / 塌缩单帧）走完整 CLI 迁移，
...
- 磁盘目录消失时 DSH 的恢复逻辑会自动创建**新 workspace 记录 + 新空会话**（实战实测发生：
...
1. **磁盘目录改名/移动**：`mv <from> <to>`（目标非空目录默认拒绝，`--merge-dir` 显式合并）。
...
10. **清理 symlink**（重启确认正常后）：`rm <from>` 与旧 projectKey 目录 symlink；
    清理后跑 `--verify` 只读校验闭环（无需第二次重启）。

## 5. 运行中进程的行为（为什么需要 symlink + 一次重启）
...
- 因此：**磁盘层立即生效（靠 symlink），完整生效必须重启一次**。重启后进程只引用新路径，
  symlink 使命结束可删；删除后的确认用 `--verify`（只读）替代第二次重启。
  `--verify` 核对：注册表记录 ↔ 会话 header cwd（原始拼写）↔ 帧不变式 ↔ 会话目录 ↔
  缓存（陈旧属自愈警告）↔ symlink 残留。零重启做不到：DSH 无在线 rehome API，
  唯一受支持的内存刷新机制就是重启。
...
| 目标路径已有 workspace 记录 | 合并 sessionIds + 删除旧记录 + 同步 workspaceIds + 清理空会话 |
| 目标路径已存在非空磁盘目录 | 默认拒绝；`--merge-dir` 显式确认（先查同名冲突） |
| symlink 残留导致 `realpath(header.cwd)` 与 record.path 不匹配 | record.path 用**真实路径**；symlink 仅作过渡并在重启后删除 |
| 无 zstd CLI 的机器 | 脚本优先 `node:zlib` 原生 zstd（Node ≥ 22.15），回退 `@mongodb-js/zstd`，再回退 CLI，全无则明确报错 |
